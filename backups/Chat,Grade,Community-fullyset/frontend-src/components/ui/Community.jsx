...
const Community = () => {
  ...
  const [isAnonQuestion, setIsAnonQuestion] = useState(true);
  ...
  const postQuestion = async () => {
    ...
    try {
      await axios.post(
        API_BASE,
        { question: newQuestion, isAnonymous: isAnonQuestion },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setNewQuestion("");
      fetchPosts();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-900 to-slate-900 text-white p-6 overflow-y-auto">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-slate-800 p-5 rounded-2xl mb-6 shadow-lg">
          <textarea
            className="w-full p-3 rounded-xl bg-slate-700 text-white border border-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            rows="3"
          ...
          <div className="flex justify-between mt-3 items-center">
            ...
            <button
              ...
              className="bg-indigo-600 px-4 py-2 rounded-lg hover:bg-indigo-700 flex items-center gap-2 transition"
            >
              <PlusCircle /> Post
            </button>
          </div>
        ...
        {/* Filter & Credit Controls */}
        <div className="flex justify-between items-center mb-5">
          <div className="flex gap-3">
            {[
              { label: "Recent", icon: <Clock size={14} />, key: "recent" },
              { label: "Connections", icon: <Users size={14} />, key: "connections" },
              { label: "My Posts", icon: <User size={14} />, key: "mine" },
            ].map(f => (
              <button
                key={f.key}
