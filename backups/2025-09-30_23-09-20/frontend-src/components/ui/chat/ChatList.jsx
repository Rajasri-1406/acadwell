...
export const authAPI = {
  login: async (data) => {
    ...
    if (res.data.success) {
      ...
      sessionStorage.setItem("user_id", res.data.user.id); // save user_id
    }
    return res;
  },
  logout: async () => {
...
export const chatAPI = {
  // Get available users for suggestions (students see teachers, teachers see students)
  getUsers: () => api.get("/users/users"),

  // Send a follow request to another user
  sendFollow: (to_id) => api.post("/users/follow", { to_id }),

  // Accept a follow request
  acceptFollow: (from_id) => api.post("/users/accept", { from_id }),

  // Reject a follow request
  rejectFollow: (from_id) => api.post("/users/reject", { from_id }),
