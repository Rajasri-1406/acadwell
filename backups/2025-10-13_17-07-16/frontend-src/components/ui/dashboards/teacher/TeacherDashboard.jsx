...
import StudyGroup from "../../StudyGroup"; // ✅ StudyGroup component
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      try {
        ...
        if (res.data.success) setTeacher(res.data.user);
      } catch (err) {
      ...
      <nav className="sidebar-nav">
        <ul>
          ...
          <li className={`nav-item ${activeTab === "chat" ? "active" : ""}`}>
            <Link to="/dashboard/teacher/chat" className="flex items-center space-x-3 w-full">
              <Users className="nav-icon" /> <span>Chats</span>
            </Link>
          </li>

          <li className={`nav-item ${activeTab === "study-groups" ? "active" : ""}`}>
            <Link to="/dashboard/teacher/study-groups" className="flex items-center space-x-3 w-full">
              <BookOpen className="nav-icon" /> <span>Study Groups</span>
            </Link>
          </li>

          <li className={`nav-item ${activeTab === "community" ? "active" : ""}`}>
            <Link to="/dashboard/teacher/community" className="flex items-center space-x-3 w-full">
  ...
  const WelcomeCard = () => (
    <div className="dashboard-card welcome-card">
      ...
      <div className="quick-actions">
        ...
        <Link to="/dashboard/teacher/chat">
          <button className="quick-action-btn add-resources">
            <Users className="action-icon" />
            <span>Chats</span>
          </button>
        </Link>
        <Link to="/dashboard/teacher/study-groups">
          <button className="quick-action-btn add-resources">
      ...
      <div className="teacher-dashboard-main">
        ...
        <div className="teacher-dashboard-content">
          <Routes>
            ...
            <Route path="chat" element={<TeacherStudentGroups />} /> {/* Chats */}
            <Route path="study-groups" element={<StudyGroup />} /> {/* Study Groups */}
