...
const BADGES_API = "http://localhost:5000/api/student/my_badges"; // now used for credits
...
const StudentWellness = () => {
  ...
  const [credits, setCredits] = useState(0); // total credits from badges
  ...
  const fetchCredits = async () => {
    if (!token) return;
    try {
      const res = await axios.get(BADGES_API, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.data.success && Array.isArray(res.data.badges)) {
        const totalCredits = res.data.badges.reduce(
