...
def get_correlation():
    ...
    # 1️⃣ Study Group Activity: % of groups joined and messages sent
    ...
    # 2️⃣ Peer Help Given: badges + answers provided to others
    badges_given = list(
        db.users.aggregate([
            {"$unwind": "$peerBadges"},
    ...
    peer_help_count = len(badges_given) + sum(1 for ans in answers if db.questions.find_one({"_id": ans.get("questionId"), "askedBy": {"$ne": user_id}}))
    peer_help_given = min(100, peer_help_count * 10)

    # 3️⃣ Answers Received / Engagement
    ...
    # 4️⃣ Private Messages to Connections (anonymity included)
