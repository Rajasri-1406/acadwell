...
def serialize_group(group):
    ...
    group.pop("createdBy", None)  # hide creator
    return group
...
def suggestions():
    try:
        ...
        public_groups = list(current_app.db.groups.find({"isPrivate": False, "members": {"$ne": user_id}}))
        private_groups = list(current_app.db.groups.find({"isPrivate": True, "createdBy": {"$in": connections}, "members": {"$ne": user_id}}))

        all_groups = [serialize_group(g) for g in public_groups + private_groups]
...
def get_group_details(group_id):
    try:
        ...
        members = [current_app.db.users.find_one({"id": uid}).get("anonId", "Anonymous") for uid in group.get("members", [])]
        group["_id"] = str(group["_id"])
