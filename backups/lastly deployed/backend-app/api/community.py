...
def get_posts():
    ...
    if filter_type == "connections":
        connection_ids = [
            c["user1"] if c["user2"] != user_id else c["user2"]
            for c in db.connections.find({"$or": [{"user1": user_id}, {"user2": user_id}]})
        ]

        if connection_ids:
            posts = list(
                db.community_posts.find({"userId": {"$in": connection_ids}})
                .sort("createdAt", -1)
            )
...
def post_answer(post_id):
    ...
    res = db.community_posts.update_one(
