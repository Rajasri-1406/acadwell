...
def find_user_by_anon_id(anon_id):
    return current_app.db.users.find_one({"anonId": anon_id})
...
def register_student():
    try:
        ...
        # Check for duplicate anonId
        if find_user_by_anon_id(data["anonId"]):
            return jsonify({"success": False, "message": "Anonymous ID already taken"}), 400

        student = {
            "id": str(data["anonId"]),  # use manually entered anonId as main id
            "anonId": data["anonId"],
            "role": "student",
            "name": data["name"],
