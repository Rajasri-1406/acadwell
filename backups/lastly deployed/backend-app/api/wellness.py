...
from dateutil.parser import parse  # for safe date parsing
from app.models import messages_col  # ensure this is defined in models.py
...
def wellness_summary():
    ...
    messages = list(messages_col.find({"senderId": user_id}, {"_id": 0, "timestamp": 1}))
    ...
    dates = sorted({
        parse(msg["timestamp"]).date() if isinstance(msg["timestamp"], str) else msg["timestamp"].date()
        for msg in messages
    }, reverse=True)
    ...
    peer_sessions = len(dates)  # active days messaging
...
def get_correlation():
    ...
    # 1️⃣ Study Group Activity
    total_groups = db.groups.count_documents({})
    joined_groups = db.groups.count_documents({"members": {"$in": [user_id]}})
    messages_in_groups = db.messages.count_documents({"senderId": user_id, "type": "group"})
    study_group_activity = min(100, ((joined_groups * 0.6 + messages_in_groups * 0.4) / max(total_groups, 1)) * 100)
    ...
    answers = list(db.answers.find({"answeredBy": user_id}))
    peer_help_count = len(badges_given) + sum(
        1 for ans in answers
        if db.questions.find_one({"_id": ans.get("questionId"), "askedBy": {"$ne": user_id}})
    )
    peer_help_given = min(100, peer_help_count * 10)

    # 3️⃣ Answers Received
    posts = db.community_posts.find({"userId": user_id})
    total_posts, total_answers = 0, 0
    for p in posts:
        total_posts += 1
        total_answers += len(p.get("answers", []))
    answers_received = round((total_answers / total_posts) * 100, 2) if total_posts else 0

    # 4️⃣ Private Messages
    ...
    anonymous_discussions = round((msgs_to_connections / total_private_msgs) * 100, 2) if total_private_msgs else 0
