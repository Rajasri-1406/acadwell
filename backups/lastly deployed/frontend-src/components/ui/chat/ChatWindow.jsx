...
const ChatWindow = ({ partner, onMoodUpdate }) => {
  ...
  useEffect(() => {
    ...
    // ---------------- Notify parent about latest mood ----------------
    if (onMoodUpdate && messages.length) {
      const lastMoodMsg = [...messages].reverse().find((m) => m.mood);
      if (lastMoodMsg) {
        onMoodUpdate(lastMoodMsg.mood); // e.g., "happy", "sad"
      }
    }
  }, [messages]);
